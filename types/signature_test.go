...
func TestNewSignatureFromBytes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: "invalid signature V value: 29",
		},
	}
...
func TestRecover(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "success: normalized v",
			giveSignature: Signature{R: sig.R, S: sig.S, V: sig.V},
			giveHash:      hash,
			want:          crypto.PubkeyToAddress(pk.PublicKey),
		},
		{
			name:          "success: non-normalized v",
			giveSignature: Signature{R: sig.R, S: sig.S, V: sig.V - SignatureVOffset},
			giveHash:      hash,
			want:          crypto.PubkeyToAddress(pk.PublicKey),
		},
		{
