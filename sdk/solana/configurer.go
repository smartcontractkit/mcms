...
func (c *Configurer) SetConfig(ctx context.Context, mcmAddress string, cfg *types.Config, clearRoot bool) (types.NativeTransaction, error) {
	...
	if err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to extract set config inputs: %w", err)
	}

	if len(signerAddresses) > config.MaxNumSigners {
		return types.NativeTransaction{}, fmt.Errorf("too many signers (max %d)", config.MaxNumSigners)
	}
	...
	if err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to preload signatures: %w", err)
	}
	...
	if err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to set config: %w", err)
	}
