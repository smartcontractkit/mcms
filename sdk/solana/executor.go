...
func (e *Executor) ExecuteOperation(
	...
	if err = json.Unmarshal(op.Transaction.AdditionalFields, &additionalFields); err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to unmarshal additional fields: %w", err)
	}
	...
	if errors.Is(err, ErrInvalidContractAddressFormat) {
		...
		if pkerr != nil {
			return types.NativeTransaction{}, fmt.Errorf("unable to get hash from base58 To address: %w", err)
		}
	}
	...
	if err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to call execute operation instruction: %w", err)
	}
...
func (e *Executor) SetRoot(
	...
	if len(sortedSignatures) > math.MaxUint8 {
		return types.NativeTransaction{}, fmt.Errorf("too many signatures (max %d)", math.MaxUint8)
	}
	...
	if err != nil {
		return types.NativeTransaction{}, fmt.Errorf("unable to set root: %w", err)
	}
