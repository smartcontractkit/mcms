...
func Test_WriteTimelockProposal(t *testing.T) {
	...
	buildProposal := func() *TimelockProposal {
	...
	tests := []struct {
		...
		},
		{
			...
			proposal: buildProposal(),
			writer:   newFakeWriter(0, errors.New("write error")),
			wantErr:  "write error",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := WriteTimelockProposal(tt.writer, tt.proposal)

			if tt.wantErr != "" {
				require.EqualError(t, err, tt.wantErr)
			} else {
				require.NoError(t, err)
				require.JSONEq(t, tt.want, tt.writer.(*strings.Builder).String())
			}
		})
	}
}
...
func TestTimelockProposal_OperationIDs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "success: computes operation ids as expected",
			proposal: *lo.Must(dummyProposalBuilder(types.TimelockActionSchedule).
				SetOperations([]types.BatchOperation{{
					ChainSelector: chaintest.Chain2Selector,
					Transactions: []types.Transaction{
						{
...
func dummyProposalBuilder(action types.TimelockAction) *TimelockProposalBuilder {
...
func buildDummyProposal(action types.TimelockAction) TimelockProposal {
	return *lo.Must(dummyProposalBuilder(action).Build())
}
...
func hashesToHexes(hashes []common.Hash) []string {
	return lo.Map(hashes, func(h common.Hash, _ int) string { return h.Hex() })
}
