...
func (s *TimelockBypassTestSuite) TestTimelockBypass() {
	...
	executor, err := cantonsdk.NewExecutor(encoder, bypasserInspector, s.participant.LedgerServices.Command, s.participant.UserID, s.participant.PartyID, cantonsdk.TimelockRoleBypasser)
	s.Require().NoError(err)
	...
	timelockInspector := cantonsdk.NewTimelockInspector(s.participant.LedgerServices.Command, s.participant.LedgerServices.State, s.participant.PartyID)
